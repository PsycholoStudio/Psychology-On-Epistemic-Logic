...
**本質主義（Essentialism）**の現代的形式化において、対象 \(x\) が本質的に性質 \(P\) を持つということは、\(\Box P(x)\)（必然的に \(x\) は \(P\) を持つ）として表現されます。例えば、「水は本質的に H₂O である」という主張は、\(\Box \forall x (\text{Water}(x) \rightarrow \text{H₂O}(x))\) として形式化できます（量化子の詳細は[第1節1.1.5](1-1-propositional-logic.md#述語論理と量化子)参照）。
...
!!! example "状況計算と変化の論理"
    ...
    **フレーム公理の例**（ブロック世界）：
    ...
    \[
    \begin{align}
    &[move(A,B)] on(A,B) \land \\
    &[move(A,B)] (C \neq A \land C \neq B \rightarrow \\
    &\quad (on(C,D) \leftrightarrow [move(A,B)]^{-1} on(C,D)))
    \end{align}
    \]
...
!!! example "反事実条件文の意味論"
    ...
    **推論パターン**：
    ...
    \[
    \begin{align}
    1. &\quad p \Box\!\to q &&\text{（前提）}\\
    2. &\quad p \Box\!\to r &&\text{（前提）}\\
    3. &\quad p \Box\!\to (q \land r) &&\text{（1,2より）}
    \end{align}
    \]
