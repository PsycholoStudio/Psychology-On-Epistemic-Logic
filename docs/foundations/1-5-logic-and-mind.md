...
**二重過程理論（Dual-Process Theory）**は、この問題に新しい視点を提供します。**システム1**（直観的、自動的、高速）とシステム2（熟慮的、統制的、低速）という二つの認知システムが並行して機能しており、それぞれ異なる推論様式を持っています。システム1はヒューリスティックに基づく近似的推論を行い、システム2はより論理的で体系的な推論を行います。
...
標準的な認識論理では、エージェントが知識について**論理的全知（Logical Omniscience）**であると仮定されます。これは以下のような原理に現れます：

1. **論理的閉包性**：もし \\(K_a p\\) かつ \\(p \\rightarrow q\\) が論理的真理ならば \\(K_a q\\)
2. **必然性の認識**：すべての論理的真理 \\(p\\) について \\(K_a p\\)
3. **無矛盾性**：\\(K_a p\\) と \\(K_a \\neg p\\) が同時に成り立たない
...
ハーバート・サイモンが提唱した**限定合理性（Bounded Rationality）**の概念は、論理的全知問題への重要な回答を提供します。現実の人間は、完全に合理的ではないが、完全に非合理的でもありません。むしろ、**認知的制約**の下で「十分に良い」解決策を見つける**満足化（Satisficing）**戦略を採用します。

認知的制約には以下のようなものがあります：

**記憶制約**により、人間はすべての情報を完全に保持・検索することができません。**ワーキングメモリ**の容量は限られており（7±2項目）、複雑な推論では情報の部分的処理が避けられません。これは様相論理的には、エージェントの知識が \\(K_a p\\) の集合として有限であることを意味します。
...
**ウェイソン選択課題**（1966年）は、この乖離を示す代表的な実験です。「カードの片面にアルファベット、もう片面に数字が書かれている。『母音の裏は偶数』という規則を検証するために、どのカードをひっくり返すべきか」という課題において、多くの人が論理的に正しい選択（E と 7）をしません。
