...
認知発達の研究では、子どもがいつ頃から論理的必然性を理解するようになるかが重要な研究テーマとなっています。ピアジェの認知発達理論（Piaget, 1952）における**形式操作期**（11歳頃〜）では、抽象的で仮想的な思考が可能になり、「もし〜だったら」という仮定的推論ができるようになります。これは様相的思考の発達と密接に関連しています。
...
これらの演算子を使うことで、単なる真偽を超えた真理の「様相（mode）」を分析することができます。
