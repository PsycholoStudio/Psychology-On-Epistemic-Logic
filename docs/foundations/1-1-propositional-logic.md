...
様相論理を学ぶ前に、その基礎となる命題論理、集合論、述語論理について確認します。この節では、命題論理の基本要素である命題変数と論理結合子の理解、真理表による厳密な意味論の習得、集合論の基礎概念とド・モルガンの法則の理解、述語論理における量化子（∀、∃）の導入と一階・二階の区別の理解、そして妥当な論証と健全な論証の重要な区別の把握を目標とします。
...
!!! summary "本節の概要"
    ...
    **述語論理と量化子**：個体と性質の関係を表現し、全称量化子（∀）と存在量化子（∃）により一般的な文を形式化する。一階と二階の区別により表現力の階層を理解する。
