...
論理結合子は、これらの命題変数を組み合わせて複雑な文を作るための道具です。否定を表す \\(\\neg\\)（「ノット」と読む）は「でない」を意味し、連言 \\(\\land\\)（「アンド」と読む）は「かつ」、選言 \\(\\lor\\)（「オア」と読む）は「または」を表現します。含意 \\(\\rightarrow\\)（「ならば」または「インプライズ」と読む）は条件関係を、同値 \\(\\leftrightarrow\\)（「同値」または「イフ・アンド・オンリー・イフ」と読む）は双方向の条件関係を示します。

これらの記号は、数学的記法として国際的に標準化されており、心理学研究の国際的な議論においても共通言語として機能します。
